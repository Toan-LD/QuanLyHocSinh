	...
	public List<OnlineCourseDTO> findByCondition(String condition,String con2) {
		...
		try(Connection conn=db.getConnection()){
			if(!condition.equalsIgnoreCase("All")) {
				query="SELECT * FROM Course c join OnlineCourse ol on c.CourseID=ol.CourseID where ? like ?";
				statement = conn.prepareStatement(query);
				statement.setString(1,condition);
				statement.setString(2,dk);
			}else {
				query="select * from Course c join OnlineCourse ol on c.CourseID=ol.CourseID";
				statement = conn.prepareStatement(query);
			}
			ResultSet rs = statement.executeQuery();
			while(rs.next()) {
